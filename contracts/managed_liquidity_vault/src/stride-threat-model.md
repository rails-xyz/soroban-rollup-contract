...
The project under review is the Soroban managed liquidity vault implemented in [contract.rs](/Users/johnsonsu/Developer/rails/soroban-rollup-contract/contracts/managed_liquidity_vault/src/contract.rs). The contract is a single-partner, single-exchange vault with three privileged roles:
...
- `Exchange`
  ...
  - co-signs principal withdrawals.
- `Owner`
  - is an upgrade-only governance address,
  - is not intended to participate in day-to-day vault operations.
...
- owner-gated upgrade behavior via `UpgradeableInternal`
...
- The `Exchange`, `FundingPartner`, and `Owner` keys are high-trust identities. Compromise of any of these keys changes the threat profile materially.
...
- The design assumes the `Owner` is a separate governance address and not one of the day-to-day operating roles.
...
| Denial of Service      | `DoS.1.R.1` - Current code intentionally requires both `Exchange` and `FundingPartner` authorization for principal withdrawal, which protects against unilateral withdrawals but creates shared liveness dependency.<br><br>`DoS.1.R.2` - We will maintain resilient exchange signing operations, backup operators, documented signing SLAs, and emergency procedures for signing-system or workflow outages that could block approvals.<br><br>`DoS.2.R.1` - Current code intentionally binds yield withdrawal to `FundingPartner` auth and principal withdrawal to the same partner, creating a hard dependency on partner liveness.<br><br>`DoS.2.R.2` - We will require the `FundingPartner` to use resilient custody and define replacement or migration procedures if its signer set becomes unavailable.<br><br>`DoS.3.R.1` - We will ensure governance has tested emergency access procedures and operational playbooks for deployment or signing outages, because loss of owner signing availability prevents defensive upgrades.<br><br>`DoS.4.R.1` - We will audit and operationally monitor the configured token contracts and avoid assets with pause, blacklist, or admin features that can unexpectedly block vault flows unless that risk is explicitly accepted.<br><br>`DoS.5.R.1` - Current code rejects under-collateralized reserve submissions rather than accepting unsafe state.<br><br>`DoS.5.R.2` - We will maintain operational alerts and reserve headroom targets so exchange staff can update reserve settings quickly during market moves without repeatedly hitting validation failures.                                                                                                                                                                                                                                                    |
| Elevation of Privilege | `Elevation.1.R.1` - Current code isolates upgrade authority to `Owner` rather than allowing `Exchange` or `FundingPartner` to upgrade directly.<br><br>`Elevation.1.R.2` - We will keep upgrade operations on a dedicated governance path with least-privilege access and approval controls independent from day-to-day business operations.<br><br>`Elevation.2.R.1` - Accepted deployment assumption: `Exchange` and `FundingPartner` keys are held by separate parties, which preserves the intended separation of duties for principal withdrawal.<br><br>`Elevation.2.R.2` - Residual risk: compromise or collusion across both parties still defeats the main withdrawal control model.<br><br>`Elevation.3.R.1` - We will document and periodically review the real-world ownership, custody providers, signer overlap, and infrastructure overlap behind all privileged roles to prevent hidden privilege collapse.<br><br>`Elevation.4.R.1` - We will treat all upgrades as a full re-review trigger for the threat model, authorization matrix, and audit assumptions before deployment.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
