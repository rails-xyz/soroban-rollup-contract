...
impl ManagedLiquidityVaultContract {
    ...
    pub fn __constructor(
        ...
        // Probing `decimals` proves both addresses are live contracts exposing
        // the SEP-41 token interface: a non-existent contract or a non-token
        // contract makes this call fail and aborts deployment.
...
///
/// Used for ledger keys introduced after the initial deployment, so an
/// instance upgraded from an earlier Wasm version reads a well-defined value
/// instead of panicking on a missing entry.
