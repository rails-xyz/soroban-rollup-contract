...
| Repudiation            | `Repudiate.1` - Moving tokens out and replacing the custody code are the two most privileged actions. Each needs a record that ties the action to the operator who authorized it, since a recovery otherwise leaves only the token contract's transfer and an upgrade only the raw transaction.<br><br>`Repudiate.2` - The chain holds the commitment; the explorer holds the evidence. The root is immutable, and the block data that gives it meaning lives in a system the exchange can rewrite, so durability of the transparency claim rests on independent archival. |
...
| Repudiation            | `Repudiate.1.R.1` - Current code emits `RecoveredEvent` with the token, destination, and amount, and `UpgradedEvent` with the operator and the new Wasm hash, matching the event coverage of every other value-moving path.<br><br>`Repudiate.1.R.2` - Retain the approval record for each recovery and upgrade off-chain, because the event carries the action and not its business justification.<br><br>`Repudiate.2.R.1` - Archive published block data immutably, content-addressed or append-only, for at least the dispute window, so the evidence behind a historical root stays fixed.<br><br>`Repudiate.2.R.2` - Consider periodically anchoring a digest of the published set, so a third party can prove what was published from the anchor alone. |
...
- `recover` and `upgrade` left no on-chain record (`Repudiate.1`). Both now emit events, so the two most privileged actions are as visible as every other value-moving path.
