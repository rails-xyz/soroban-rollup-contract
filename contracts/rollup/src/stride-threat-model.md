...
| Tampering              | `Tamper.1.R.1` - Current code rejects the zero hash and a repeat of the current hash, and enforces `old_block_hash`, which keeps settlements replay-free and in order.<br><br>`Tamper.1.R.2` - Accepted design: settlement truth is verified off-chain. Define the root over a versioned, published encoding, and publish each preimage with or before the `rollup` transaction so verification is possible at settlement time.<br><br>`Tamper.1.R.3` - Run an independent verifier that recomputes each committed root from published data and alerts on a mismatch, so a discrepancy is found internally before a user finds it.<br><br>`Tamper.2.R.1` - Planned change: store a block height beside `LatestBlockHash`, increment it in each `rollup`, and carry it in `NewBlockEvent`. The contract reads the root as opaque bytes, so verification stays off-chain; the height gives every settlement a canonical sequence number, which places a re-posted root in a numbered published history rather than one an observer reconstructs. The `rollup` signature is unchanged and the event schema gains a field.<br><br>`Tamper.2.R.2` - Alert on any `new_block_hash` committed before, and treat a repeat as a settlement incident. This is the standing detection path, run alongside the root verifier in `Tamper.1.R.3`.<br><br>`Tamper.3.R.1` - Deploy only against a vetted token with exact transfer semantics and no administrative control over held balances, and keep the token contract in audit scope. This is the whole treatment.<br><br>`Tamper.4.R.1` - Current code restricts `upgrade` to the owner and rejects any operator that differs from it.<br><br>`Tamper.4.R.2` - Require a published Wasm hash, independent review, and post-upgrade verification of the authorization and coverage invariants. Every upgrade re-triggers this threat model.<br><br>`Tamper.5.R.1` - Current code rejects `recover` when `token_address` equals the configured `CollateralToken`.<br><br>`Tamper.5.R.2` - Confirm at deployment that the configured collateral asset is reachable through that address alone, and repeat the check whenever the token choice changes. Apply the same approval to `recover` as to `collect_fees`. |
| Repudiation            | `Repudiate.1.R.1` - Planned change: emit events from `recover` and `upgrade`, carrying the token, destination, and amount, and the operator and new Wasm hash.<br><br>`Repudiate.1.R.2` - Until then, register every recovery and upgrade off-chain with its approval, reason, and transaction hash.<br><br>`Repudiate.2.R.1` - Archive published block data immutably, content-addressed or append-only, for at least the dispute window, so the evidence behind a historical root stays fixed.<br><br>`Repudiate.2.R.2` - Consider periodically anchoring a digest of the published set, so a third party can prove what was published from the anchor alone. |
...
**Did the STRIDE model uncover new design issues?** Yes, three worth acting on:

- The explorer is a security component (`Spoof.3`, `Repudiate.2`, `DoS.4`). Transparency is the product, and it rests on a system that sits off-chain and inside the exchange. It needs to be operated, monitored, and archived to that standard.
- `recover` and `upgrade` emit no events (`Repudiate.1`). The two most privileged actions leave the weakest record.
- A committed root can be re-posted (`Tamper.2`). A block height in storage and in the event numbers each settlement, so a re-post is visible in the published history. Detection stays with the verifier, because the contract reads the root as opaque bytes.
...
The largest residuals are accepted by design and named as such: full exchange authority, off-chain root verification, exchange-serviced exits, and off-chain solvency. Three small code changes are worth making: events on `recover` and `upgrade`, and a block height. The pause and the upgrade timelock are deliberate trade-offs.

The load-bearing treatments are procedural: settlement alerting (`Spoof.1.R.3`) and independent archival of published block data (`Repudiate.2.R.1`). Where transparency carries the accountability, detection latency and durable publication are the controls, and should be resourced as such.
